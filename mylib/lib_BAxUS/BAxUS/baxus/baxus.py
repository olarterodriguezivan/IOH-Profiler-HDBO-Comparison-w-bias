...
class BAxUS(EmbeddedTuRBO):
    ...
    @sample_zero.setter
    def sample_zero(self,new_sample:bool)->None:
        """
    ...
    def _resample_and_restart(self, n_points: int, length: float = None) -> None:
        ...
        X_init = one_around_origin_latin_hypercube(n_pts, self.target_dim, self.sample_zero)

        if isinstance(self.projector,AxUS):
            X_init_up = from_1_around_origin(
                self.projector.project_up(X_init.T).T, self.lb, self.ub
            )
        else:
            # This is an addition in case the Projector is not set
            X_init_up = X_init.copy()
