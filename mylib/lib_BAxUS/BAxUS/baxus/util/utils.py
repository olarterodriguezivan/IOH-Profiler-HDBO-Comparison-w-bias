...
def from_1_around_origin(x: np.ndarray, lower_bounds: np.ndarray, upper_bounds: np.ndarray) -> np.ndarray:
    ...
    Args:
        ...
        lower_bounds: the lower bounds of the full space
        upper_bounds: the upper bounds of the full space
        sample_zero: A switch to include to sample the zero vector from LHS
...
def one_around_origin_latin_hypercube(n_pts: int, dim: int, sample_zero: bool) -> np.ndarray:
    """
    ...
    Args:
        ...
        sample_zero: sample the zero from the initial/repair LHS
