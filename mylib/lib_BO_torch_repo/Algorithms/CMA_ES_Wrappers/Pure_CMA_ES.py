...
class Pure_CMA_ES(AbstractAlgorithm):
    ...
    def __init__(self,
        ...
        # Use the superclass from the Abstract Algorithm
        ...
        # Store the budget as some member variable
        self.budget:int = budget

        # Store the random seed
        self.random_seed:int = random_seed

        # Set the options for the CMA-ES
        self.inopts = inopts
    ...
    def budget(self, new_budget:int)->None:
        if isinstance(new_budget,int) and new_budget > 0:
            ...
            raise ValueError("The budget must be an integer greater than zero!")
    ...
    @x0.setter
    def x0(self,new_x0:Union[List[float],np.ndarray]):
        if isinstance(new_x0,list) or isinstance(new_x0,np.ndarray):
            # Convert the list to array
            new_x0 = np.array(new_x0).ravel()
            
            if self.dimension is None:
                self.dimension = new_x0.size
            
            # Assign the new value
            self.__x0 = new_x0
        else:
            raise AttributeError(f"The argument {new_x0} is not of class `np.ndarray` or a List of `float`")
    ...
    @sigma0.setter
    def sigma0(self, new_sigma_0:float)->None:
        if isinstance(new_sigma_0,(float,int)) and new_sigma_0 > 0:
            self.__sigma0 = new_sigma_0
        else:
    ...
    @inopts.setter
    def inopts(self, new_inopts:Union[dict,CMAOptions,list])->None:
    ...
    def random_seed(self, new_random_seed:int)->None:
        if isinstance(new_random_seed,int):
            ...
            raise AttributeError("The random seed must be an integer")
    ...
    @iterations.setter
    def iterations(self, new_iterations:int)->None:
        if isinstance(new_iterations,int) and new_iterations >= 0:
            self.__iterations = new_iterations
        else:
            raise AttributeError("The number of iterations must be an integer greater than zero")
