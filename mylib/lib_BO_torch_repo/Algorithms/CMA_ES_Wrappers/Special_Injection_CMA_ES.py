...
class SpecialInjectionCMA_ES(Pure_CMA_ES):
    ...
    def __init__(self,
        ...
        # Correct the options
        self._correct_inopts()

    def _correct_inopts(self):
        r"""
        This function corrects the options for the CMA-ES
        """

        # Since there is a need for injection, then, the Active CMA-ES should be deactivated.
        self.__inopts.set("CMA_active",False)
            ...
            if 1 < 3:  # try: # pass on KeyboardInterrupt
                ...
                while True:  # restart loop
                    ...
                    while not es.stop():  # iteration loop
                        ...
                        new_X, is_sorted = sorting_function(X)
                        checkPoints = None
                        if np.all(np.array(is_sorted,dtype=bool)) == False:
                            # If the array is not sorted then check the points to clip
                            checkPoints = np.where(np.array(is_sorted,dtype=bool) == False)[0].ravel().tolist()
                            
                        es.tell(new_X, fit, check_points=checkPoints)  # prepare for next iteration
                        for f in callback:
                            f is None or f(es)
                        es.disp()
