...
class ModularNonBBOBProblemWithSubspace(ioh.iohcpp.problem.RealSingleObjective):
    """
    ...
    def evaluate(self, 
        ...
        if self.mode == "average":
            average_value:float = x_reshape.mean(axis=None)

            # Compute the intrinsic function
            return self.__intrinsic_problem([average_value])

        # This is to set in case the mode is set as the maximum
        elif self.mode == "maximum":
            
            max_value:float = x_reshape.max(axis=None)
            
            # Compute the intrinsic function
            return self.__intrinsic_problem([max_value])
